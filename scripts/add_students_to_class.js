...
async function addStudentsToClass10A() {
    try {
        ...
        const classroom = await Classroom.findOne({ name: 'Class 10A', school: school._id });
        if (!classroom) {
        ...
        const studentsToAdd = [
            { name: 'Ram Sharma', email: 'ram@student.com', parentEmail: 'ramparent@gmail.com' },
            { name: 'Sita Thapa', email: 'sita@student.com', parentEmail: 'sitaparent@gmail.com' },
            { name: 'Hari Gurung', email: 'hari@student.com', parentEmail: 'hariparent@gmail.com' },
            { name: 'Gita Rai', email: 'gita@student.com', parentEmail: 'gitaparent@gmail.com' },
            { name: 'Shyam Magar', email: 'shyam@student.com', parentEmail: 'shyamparent@gmail.com' },
        ];

        console.log('Adding students to Class 10A...');
        for (const s of studentsToAdd) {
            const existing = await Student.findOne({ email: s.email, school: school._id });
            if (existing) {
