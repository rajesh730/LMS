...
export async function GET(_request, props) {
  try {
    ...
    if (!schoolConfig?.allowStudentGlobalWall) {
      return NextResponse.json(
        { message: "Global Wall is not enabled" },
        { status: 403 }
      );
    }
    ...
    if (!mongoose.Types.ObjectId.isValid(params.id)) {
      return NextResponse.json(
        { message: "Global Wall article not found" },
        { status: 404 }
      );
    }
    ...
    console.error("GET /api/student/global-wall/[id] error:", error);
    return NextResponse.json(
      { message: "Failed to load Global Wall article" },
      { status: 500 }
    );
  }
}
