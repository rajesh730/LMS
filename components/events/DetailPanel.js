  ...
  const handleRemoveStudent = async (studentId) => {
    ...
    try {
      ...
      const res = await fetch(
        `/api/events/${event._id}/manage/student/${studentId}`,
        { method: "DELETE" }
      );
      ...
      <div className="flex-1 overflow-y-auto mb-6">
        {filteredRequests.length === 0 ? (
          <div className="text-center py-12">
            <p className="text-gray-500 mb-2">No {status.toLowerCase()} requests found</p>
      ...
      {showRejectionForm && status === "PENDING" && (
        <div className="mb-6 p-4 bg-red-50 border border-red-200 rounded-lg">
          ...
          <div className="flex gap-2">
            ...
            >
              Reject {selectedStudents.length > 0 ? `(${selectedStudents.length})` : ""}
            </button>
            <button
              onClick={() => {
                setShowRejectionForm(false);
