...
export default function SchoolSubjectManager() {
  ...
  const customSubjects = subjects.filter((s) => s.subjectType === "SCHOOL_CUSTOM");
    ...
    <div className="space-y-8">
      ...
      <div className="flex gap-2">
        <button
          ...
          title="Download custom subjects as CSV"
        >
          <FaDownload /> Download
        </button>
        <button
      ...
      <div>
        <h3 className="text-lg font-semibold text-white mb-4">Global Subjects (Platform)</h3>
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {globalSubjects.map((subject) => (
            ...
            >
              ...
              <p className="text-xs text-slate-500 mt-2">{subject.academicType}</p>
            </div>
          ))}
        </div>
        <p className="text-sm text-slate-400 mt-2">
          These subjects are available platform-wide and can be used for your grades.
        </p>
      </div>
      ...
      <div>
        <div className="flex items-center justify-between mb-4">
          <h3 className="text-lg font-semibold text-white">Custom Subjects (School)</h3>
          <button
            onClick={() => {
            ...
            className="flex items-center gap-2 px-4 py-2 bg-emerald-600 hover:bg-emerald-700 text-white rounded-lg transition text-sm"
          >
        ...
        {/* Messages */}
        {error && (
          <div className="bg-red-500/10 border border-red-500/30 p-4 rounded-lg text-red-400 mb-4">
            {error}
          </div>
        )}
        {success && (
          <div className="bg-green-500/10 border border-green-500/30 p-4 rounded-lg text-green-400 mb-4">
            {success}
          </div>
        )}
        ...
        {/* Form */}
        {showForm && (
          <div className="bg-slate-800 border border-slate-700 p-6 rounded-lg space-y-4 mb-6">
        ...
        {customSubjects.length > 0 ? (
          ...
          <div className="text-center py-8 text-slate-400">
            No custom subjects created yet. Create one to get started!
          </div>
        )}
      </div>
    </div>
