...
export default function GlobalCurriculumManager() {
  ...
  const [newSubjectData, setNewSubjectData] = useState({
    educationLevel: 'School', // Default to School
    selectedGrades: [],
    // Tabular Data
    subjects: [{ name: '', code: '', academicType: 'CORE', description: '' }]
  });

  // Filters
  const [searchTerm, setSearchTerm] = useState('');
  const [activeLevel, setActiveLevel] = useState('ALL'); // ALL, School
  const [activeGrade, setActiveGrade] = useState('ALL');
  
  const GRADE_OPTIONS = Array.from({ length: 10 }, (_, i) => String(i + 1));
                    ...
                    <div className="grid grid-cols-1 gap-4">
                        {[
                            ...
                            <button
                                key={level.id}
                                onClick={() => {
                                    setNewSubjectData({ ...newSubjectData, educationLevel: level.id, selectedGrades: [] });
                                    setCreationStep(2);
                                }}
                    ...
                    <div className="space-y-6">
                        ...
                        <div className="flex justify-between pt-4">
                            ...
                            <button
                                ...
                                disabled={newSubjectData.selectedGrades.length === 0}
