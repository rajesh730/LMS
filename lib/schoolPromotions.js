...
export async function getActiveSchoolPromotions(
  ...
  const activeSchoolCandidates = candidates.filter((promotion) => promotion.school);
  if (activeSchoolCandidates.length === 0) return [];

  const schoolIds = activeSchoolCandidates.map((promotion) => promotion.school._id);
  ...
  const eligibleCandidates = activeSchoolCandidates.filter((promotion) =>
    profileMap.has(String(promotion.school._id))
  );
