...
export const PROMOTION_STATUSES = ["DRAFT", "ACTIVE", "PAUSED", "ARCHIVED"];
...
function formatPromotion(promotion, profileMap = new Map()) {
  ...
  return {
    ...
    title: promotion.title || promotion.school?.schoolName || "School Spotlight",
    tagline:
      promotion.tagline ||
      profile?.tagline ||
      "A promoted school spotlight from the Pratyo network.",
...
export async function getActiveSchoolPromotions(
  ...
  const eligible = candidates
    .filter((promotion) => promotion.school)
    .sort((a, b) => {
