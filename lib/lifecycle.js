...
export function buildMagazineLifecycle(article = {}) {
  return cleanTimeline([
    timelineItem({
      label: article.status === "DRAFT" ? "Private draft saved" : "Posted to school wall",
      status: article.status === "DRAFT" ? "DRAFT" : "SUBMITTED",
      at: article.firstSubmittedAt || article.submittedAt || article.createdAt,
      description:
        article.status === "DRAFT"
          ? "Student kept this writing private."
          : "Student shared this writing with the school.",
    }),
    timelineItem({
      label: "Updated after posting",
      status: "RESUBMITTED",
      at: article.lastResubmittedAt,
      description: `Update ${Number(article.revisionCount || 0)} saved after posting.`,
    }),
    timelineItem({
      label:
      ...
      description: article.reviewNote || "School publishing state recorded.",
    }),
    timelineItem({
      label: "Published to homepage",
      status: "PUBLISHED",
      at: article.publishedAt,
      description: "Visible on the school public page.",
    }),
    timelineItem({
      label: "Published to school magazine",
      status: "PUBLISHED",
      at: article.magazinePublishedAt,
      description: "Visible in the school magazine.",
    }),
    timelineItem({
      label: "Last updated",
