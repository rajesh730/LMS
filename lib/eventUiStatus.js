...
export function formatEventDate(value, fallback = "Date to be announced") {
...
export function getEventPublicStatus(event = {}) {
  ...
  if (registrationDeadline && !isDatePast(registrationDeadline, { endOfDay: true })) {
    return {
  ...
  if (lifecycleStatus === "ACTIVE" && event.date && !isDatePast(event.date, { endOfDay: true })) {
    return {
      key: "registration-open",
      label: "Upcoming",
