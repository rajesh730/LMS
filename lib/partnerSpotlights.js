...
function formatPartnerSpotlight(organizer, organizerEvents = []) {
  ...
  return {
    ...
    description:
      ...
      "Selected event partner on the platform.",
...
async function getAdminSelectedSpotlights(limit) {
  const organizers = await ExternalOrganizer.find({
    ...
    .select(
      "organizationName slug logoUrl website location description trustLevel partnerRoles spotlightPriority spotlightLastShownAt spotlightImpressionCount updatedAt"
    )
    .sort({ updatedAt: -1 })
    .limit(100)
    .lean();
  ...
  const eligible = organizers
    ...
    .filter((spotlight) => spotlight.slug && spotlight.activeEventCount > 0)
    .sort((a, b) => {
      const priorityDiff =
