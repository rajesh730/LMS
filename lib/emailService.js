...
/**
 * Email delivery of a school notice to a guardian (§31).
 *
 * Email is a DELIVERY CHANNEL, never the source of truth. Three consequences
 * are visible in this function:
 *
 *  - It sends a short summary plus a link, not the whole notice with its
 *    action buttons. Consent and acknowledgement must be recorded against an
 *    authenticated guardian in Pravyo, so the email deliberately cannot be
 *    "answered" — it drives the parent back to the app.
 *  - It never touches the NoticeReceipt. Sending an email is not the guardian
 *    reading the notice, and nothing here may set `openedAt`.
 *  - Like every other send in this module it resolves rather than throwing, so
 *    a mail outage can never fail a school's publish action.
 */
export async function sendNoticeEmail({
  ...
  const noticeUrl = `${SITE_URL}/parent/notices/${noticeId}`;
  const preview = String(noticeBody || "").slice(0, 400);

  return deliver({
    to,
    subject: requiresAction
      ? `Action needed: ${noticeTitle} - ${studentName}`
      : `${noticeTitle} - ${studentName}`,
    html: wrapEmailBody(
      noticeTitle,
      `
        <p>Dear ${guardianName || "Parent"},</p>
        <p>
          ${schoolName} has sent a notice about
          <strong>${studentName}</strong>.
        </p>
        <div style="background:#f5f7fa;border-left:4px solid #1f4e79;padding:12px 16px;margin:16px 0;">
          <p style="margin:0;white-space:pre-wrap;">${preview}</p>
        </div>
        ${
          requiresAction
            ? `<p style="color:#b42318;font-weight:bold;">
                 This notice needs your response. Please open Pravyo to reply.
               </p>`
            : ""
        }
        <p><a class="button" href="${noticeUrl}">Open in Pravyo</a></p>
        <p style="font-size:13px;color:#666;">
          Signing in with your Parent ID and PIN keeps your child's information
          private. If you cannot sign in, please contact the school office.
        </p>
      `
    ),
  });
