...
function startRedisSubscription(listener) {
  // A new browser subscription needs changes from now onward. Starting at "-"
  // replayed the oldest stream entries on every connection, causing needless
  // refetches and making historical notification events look new. Initial page
  // state is loaded through its API before realtime takes over.
