...
const REDIS_STREAM_KEY = "pratyo:realtime:events";
const REDIS_POLL_INTERVAL_MS = 1500;
const REDIS_BATCH_SIZE = 100;

function getMetrics() {
  if (!globalThis.__pratyoRealtimeMetrics) {
    globalThis.__pratyoRealtimeMetrics = {
  ...
  return globalThis.__pratyoRealtimeMetrics;
}

function getBus() {
  if (!globalThis.__pratyoRealtimeBus) {
    globalThis.__pratyoRealtimeBus = new EventEmitter();
    globalThis.__pratyoRealtimeBus.setMaxListeners(100);
  }

  return globalThis.__pratyoRealtimeBus;
}
