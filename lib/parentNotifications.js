 ...
 *
 * Callers should await this promise but contain failures, so serverless
 * runtimes stay alive long enough to hand push to the provider without letting
 * an alert failure undo the action that triggered it.
 */
export async function notifyGuardians({
  ...
  // The durable in-app row is the source of truth; web push is the wake-up
  // signal for installed phones. Delivery errors are contained inside the push
  // service and never undo the notification that was just stored.
  await sendPushToParents(
    links.map((link) => link.parent),
    {
      title: cleanTitle,
      body: cleanMessage,
      href: href || "/parent/notifications",
      tag: `${category}-${studentId}`,
      urgent: priority === "URGENT" || priority === "ACTION",
    }
  );
