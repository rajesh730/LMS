...
function pickPreferredRoundStatus(current, next) {
  const currentPriority = ROUND_VIEW_STATUS_PRIORITY[current] || 99;
  const nextPriority = ROUND_VIEW_STATUS_PRIORITY[next] || 99;
  ...
  for (const participant of withSubmissions) {
    ...
    if (!teamMap.has(teamKey)) {
      teamMap.set(teamKey, {
        ...
        status: participant.status || "PARTICIPATED",
...
export async function syncApprovedRequestsToRoundOne({
  ...
  if (created) {
    await EventNotice.create({
      ...
      message:
        "Approved participants were added automatically to Round 1. Schools can review round details in the event workspace.",
...
export async function advanceSelectedParticipants({
  ...
  await EventNotice.create({
    event: eventId,
    round: round._id,
    title: `${round.title} progression updated`,
    message: `${movedEntriesCount} selected ${
      isTeamParticipationEvent(event) ? "team(s)" : "participant(s)"
    } were moved to ${targetLabel}.`,
    type: "SHORTLIST",
    targetAudience: "REGISTERED_SCHOOLS",
    visibility: "PUBLIC",
    status: "PUBLISHED",
