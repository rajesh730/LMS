...
// A small list of selectable AD start years for settings dropdowns, centered on
// the current AD year.
export function buildAcademicYearOptions(calendar = "AD", { back = 6, forward = 1 } = {}) {
  const current = getCurrentAdYear();
  const options = [];
  for (let start = current - back; start <= current + forward; start += 1) {
    options.push(buildAcademicYear(start, calendar));
  }
