...
async function connectDB() {
  ...
  if (!cached.promise) {
    const opts = {
      ...
      // 15s (not 5s): a paused Atlas M0 can take longer than 5s to resume,
      // and a too-short timeout turns the first visit after idle into an
      // error page instead of a slow-but-successful load.
