...
/**
 * How long a SHARED-device session stays trusted between PIN entries (§12).
 *
...
export async function requireParentSession() {
  ...
  // Shared-device idle timeout. A distinct code so the app can show a
  // "re-enter your PIN" screen rather than dumping the guardian at a full
  // sign-in they may not know how to complete.
  if (session.user.deviceMode === "SHARED") {
    const verifiedAt = Number(session.user.pinVerifiedAt) || 0;
    if (!verifiedAt || Date.now() - verifiedAt > SHARED_DEVICE_IDLE_MS) {
      return {
        error: errorResponse(
          401,
