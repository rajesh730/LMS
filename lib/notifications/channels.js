  ...
  async send({ notice, recipients, priority }) {
    try {
      ...
      const result = await createParentNotificationsForTargets({
        targets: reachable.map(({ parent, student }) => ({
          parentId: parent._id,
          studentId: student._id,
        })),
        type: notice.requiresConsent ? "CONSENT" : "NOTICE",
        priority:
          priority === "URGENT"
            ? "URGENT"
            : priority === "IMPORTANT"
              ...
              : priority === "POSITIVE"
                ...
                : "INFO",
        title: notice.title,
        body: String(notice.content || "").slice(0, 200),
        actionUrl: `/parent/notices/${notice._id}`,
        entityId: String(notice._id),
        dedupeKey: `NOTICE:${notice._id}`,
        metadata: { noticeId: String(notice._id) },
      });

      return { channel: this.key, status: "SENT", count: result.sent };
    } catch (err) {
